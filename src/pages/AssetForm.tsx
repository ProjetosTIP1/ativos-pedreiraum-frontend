...
export const AssetForm: React.FC = () => {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const file = files[0];
      const imageMetadata = await imageService.uploadAnImage(file, id);

      console.log(
        "Image uploaded successfully, updating form data:",
        imageMetadata,
      );
    } catch (error) {
  ...
  const handleRemoveImage = (imageId: string) => {
    setFormData((prev) => {
      ...
      // If the removed image was main and there are others left, pick a new main
      if (
        updatedImages.length > 0 &&
        !updatedImages.some((img) => img.is_main)
      ) {
        ...
        <div className={style.mainColumn}>
          ...
          <div className={style.card}>
            ...
            <p className={style.helpText}>
              <AlertCircle size={10} className="mr-1" /> Clique em uma imagem
              para defini-la como principal. A primeira imagem enviada será
              definida como principal automaticamente.
            </p>
          </div>
        </div>
