  ...
  if (error || !currentAsset) {
    return (
      <div className={style.errorState}>
        ...
        <Button onClick={() => navigate("/ativos")}>
...
Poderia me passar mais informações sobre disponibilidade e valor?`;
  ...
  const images =
    currentAsset.images && currentAsset.images.length > 0
      ? currentAsset.images
      : currentAsset.main_image
        ? [
            {
              url: currentAsset.main_image,
              is_main: true,
              id: "main",
              name: "Main",
            },
  ...
  return (
    <div className={style.container}>
      {/* Breadcrumb / Back */}
      <button onClick={() => navigate("/ativos")} className={style.backButton}>
      ...
      <div className={style.mainGrid}>
          ...
          <div className={style.thumbnailsGrid}>
            {galleryUrls.map((img, idx) => (
              ...
              >
                <img
                  src={img}
                  alt={`${currentAsset.name} thumb ${idx}`}
        ...
        <div className={style.infoWrapper}>
          <div className={style.infoHeader}>
            ...
            <h1 className={style.assetTitle}>{currentAsset.name}</h1>
            <div className={style.metaBar}>
              <span className={style.metaItem}>
                <Calendar size={14} className={style.metaIcon} />{" "}
                {currentAsset.year}
              </span>
              <span className={style.metaItem}>
                <MapPin size={14} className={style.metaIcon} />{" "}
          ...
          <div className={style.priceActionBox}>
            <div className={style.priceWrapper}>
              ...
              <span className={style.priceValue}>
                {currentAsset.price
                  ? `R$ ${currentAsset.price.toLocaleString("pt-BR")}`
                  : "Consultar Valor"}
              </span>
            </div>
            <Button
            ...
            {isMachine && currentAsset.specifications?.hours && (
              <div className={style.specItem}>
                ...
                <span className={style.specValue}>
                  ...
                  {currentAsset.specifications.hours}h
                </span>
              </div>
            )}
            {isVehicle && currentAsset.specifications?.mileage && (
              <div className={style.specItem}>
                ...
                <span className={style.specValue}>
                  ...
                  {currentAsset.specifications.mileage.toLocaleString()} KM
                </span>
              </div>
            )}
          </div>
          ...
          {/* Description */}
          <div>
            <label className={style.descriptionLabel}>
              Descrição & Observações
            </label>
            <p className={style.descriptionText}>{currentAsset.description}</p>
          </div>
        </div>
      </div>
